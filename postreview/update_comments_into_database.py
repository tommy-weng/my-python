...
def need_update(sha1, comment_db, note_db):
    commits_db = Recorder("commits")
    commit = commits_db.query("sha1='%s'" % sha1)
    if 0 == len(commit):
        return False
    reviewed = commit[0][9]
    comments = comment_db.query("sha1='%s'" % sha1)
    if 0 == len(comments) and not reviewed:
        return True
...
if __name__ == '__main__':
    ...
    #all_logs = save_all_logs("2019-1-15", tracking_info)
    all_logs = save_all_logs("2017-10-1", tracking_info)
    
    comments = Recorder("comments")
    notes_db = Recorder("notes")

    #print need_update("5132c1b438d8622027c11b10d81198f0f2b66dbb", comments, notes_db)
